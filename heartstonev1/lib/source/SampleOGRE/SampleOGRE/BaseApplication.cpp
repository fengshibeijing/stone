...
bool BaseApplication::frameRenderingQueued(const Ogre::FrameEvent& evt)
{
	...
	if (frame && frame->updated)
	{
		for (int i=0;i<10;i++)
		{
			if (sdk.GetFinger(i))
			{
				fingerArray[i]->setVisible(true);
				fingerTrails[i]->setVisible(true);
				fingerArray[i]->setPosition(-sdk.GetFinger(i)->position.x*100,sdk.GetFinger(i)->position.z*100,sdk.GetFinger(i)->position.y*100);
			}
			else
			{
				fingerArray[i]->setVisible(false);
				fingerTrails[i]->setVisible(false);
			}
		}
		for (int i=0;i<2;i++)
		{
			if (sdk.GetHand(i))
			{
				handArray[i]->setVisible(true);
				handArray[i]->setPosition(-sdk.GetHand(i)->position.x*100,sdk.GetHand(i)->position.z*100,sdk.GetHand(i)->position.y*100);
			}
			else
			{

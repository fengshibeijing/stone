...
namespace sharpnow
{
    ...
    public:
        ...
        //! @brief 开启并配置VR环境
        inline void ConfigVR(VRConfig* config) { if (mIsValid) mConfigVR(config); }
        ...
        inline bool LoadInPath(const char* path)
        {
            ...
            mConfigVR       = (Proto_ConfigVR)GetProcAddress(mDLL, "ConfigVR");
            mIsValid = mRetrieveFrame && mEmulateHID && mGetFinger && mGetFingerFocus && mGetHand && mGetHandFocus && mConfigVR;
            return mIsValid;
        }
    private:
